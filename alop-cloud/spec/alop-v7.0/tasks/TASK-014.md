...
## Business Goal
Receivable, Collection, Allocation, Reversal, Advance, Deposit accounting, double-entry ledger, local strong transaction.
...
## V6.3 Payment/Refund Integration Addendum
Finance implementation must additionally provide:
- authoritative `QuotePaymentTargets` internal API for PaymentOrder creation;
- `RefundAmountReservation` + target details;
- idempotent Reserve/Confirm/Release refund APIs;
- consume `payment.refund.succeeded.v1` exactly once;
- UNKNOWN refund reservations remain RESERVED until provider result is definitive;
- concurrent refund reservation test proves total reserved/confirmed amount cannot exceed financially refundable amount.
