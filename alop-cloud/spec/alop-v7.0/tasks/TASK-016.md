...
## Business Goal
Invoice application, quota reservation, NuoNuo adapter, UNKNOWN query, red flush, reissue.

## Scope / Bounded Context
`alop-invoice + alop-finance`
...
## V6.4 Integration Requirement
Invoice implementation must expose `InvoiceDeliveryInstruction` and secure internal read API required by TASK-025. Invoice issuance must publish `invoice.invoice.delivery-requested.v1` when auto-email policy applies. Email delivery failure must never mutate Invoice `ISSUED` tax fact.
